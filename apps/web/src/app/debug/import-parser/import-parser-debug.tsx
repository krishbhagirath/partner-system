  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        <label className="text-sm font-bold text-ink" htmlFor="debug-import-html">
          Raw HTML
        </label>
        <textarea
          className="min-h-[520px] w-full resize-y border border-rule-strong bg-surface p-4 font-mono text-sm leading-6 text-ink shadow-sm outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/15"
        ...
        />
        <button
          className="inline-flex min-h-11 items-center justify-center bg-brand px-5 text-sm font-bold text-white transition hover:bg-brand-dark disabled:cursor-not-allowed disabled:bg-rule-strong"
          disabled={isParsing}
          type="submit"
        >
      ...
      <section className="min-w-0">
        ...
        <div className="border border-rule bg-surface shadow-sm">
          <div className="flex items-center justify-between gap-4 border-b border-rule px-4 py-3">
            ...
            <span className="text-sm font-bold text-ink-soft">{sections.length} rows</span>
          </div>
            ...
            <table className="min-w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-rule">
                {sections.map((section, index) => (
                  ...
                  >
                    {columns.map((column) => (
                      <td className="max-w-[280px] px-3 py-3 text-ink" key={column.key}>
          ...
          {hasParsed && sections.length === 0 ? (
            <div className="border-t border-rule px-4 py-6 text-sm font-semibold text-ink-soft">
